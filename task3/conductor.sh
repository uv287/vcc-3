...
run()
{
    ...
    # Bind mount /dev within the container root fs
    ...
    unshare --fork -u -p -n -m -i \
        chroot "$CONTAINERDIR/$NAME/rootfs" /bin/bash -c \
        "mount -t proc proc /proc && mount -t sysfs sys /sys && chmod 755 / && $INIT_CMD_ARGS"
}
...
exec()
{
    ...
    nsenter -u -p -n -m -i -r -w --target $CONTAINER_INIT_PID $EXEC_CMD_ARGS
...
addnetwork()
{
    ...
    # Enable interface within the container
